...
const getPlanTypeBadgeStyle = (planType?: string) => {
  ...
  if (lower.includes('ultra')) {
    return 'bg-linear-to-r from-amber-500/10 to-orange-500/10 text-amber-600 dark:text-amber-400 border-amber-300/60 dark:border-amber-700/60'
  }
  if (lower.includes('pro')) {
    return 'bg-linear-to-r from-indigo-500/10 to-purple-500/10 text-indigo-600 dark:text-indigo-400 border-indigo-300/60 dark:border-indigo-700/60'
  }
  ...
  <div class="space-y-6">
    ...
    <div class="rounded-3xl border border-slate-200/70 dark:border-white/8 bg-white/70 dark:bg-zinc-900/75 backdrop-blur-2xl p-5 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.3)] border-t border-t-white/80 dark:border-t-white/10 space-y-4">
      <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-2.5 shrink-0">
          <a-button
            ...
            class="inline-flex items-center gap-1.5 font-semibold rounded-xl! text-xs h-9! px-4! active:scale-[0.98] transition-all duration-100 ease-out"
          >
            <template #icon>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2">
        <div class="p-3 rounded-2xl bg-slate-100/60 dark:bg-zinc-800/40 border border-slate-200/50 dark:border-white/6 backdrop-blur-md flex items-center justify-between transition-all duration-200 hover:border-emerald-300/50">
        ...
        <div class="p-3 rounded-2xl bg-slate-100/60 dark:bg-zinc-800/40 border border-slate-200/50 dark:border-zinc-700/50 backdrop-blur-md flex items-center justify-between transition-all duration-200 hover:border-sky-300/50">
          <div class="space-y-0.5">
            ...
            <div class="text-lg font-black font-mono tracking-tight text-cyan-600 dark:text-cyan-400">{{ stats.tokenPlaneCount }}</div>
          </div>
      ...
      <div class="flex flex-col sm:flex-row items-center justify-between gap-3 pt-1 border-t border-slate-100 dark:border-zinc-800/80">
          ...
          <div class="p-1 rounded-2xl bg-slate-200/50 dark:bg-zinc-800/60 backdrop-blur-md border border-black/5 dark:border-white/5 flex items-center gap-1 text-xs">
            <button
              ...
              :class="[
                ...
                filterType === 'all'
                  ? 'bg-white dark:bg-zinc-900 text-emerald-600 dark:text-emerald-400 shadow-[0_2px_8px_rgba(0,0,0,0.08)] dark:shadow-[0_2px_8px_rgba(0,0,0,0.4)]'
        ...
        <div class="flex items-center gap-2.5 w-full sm:w-auto">
          <div class="relative flex-1 sm:w-64">
            <Search class="w-3.5 h-3.5 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none z-10" />
            <a-input
              v-model:value="searchQuery"
              placeholder="搜索名称 / 邮箱 / Provider..."
              allow-clear
              class="pl-9! rounded-xl! text-xs h-8.5!"
            />
          </div>
          ...
          <a-select
            ...
            class="min-w-42.5! rounded-xl! text-xs h-8.5!"
    ...
    <!-- 加载中状态 -->
    <div v-if="quotaStore.loading" class="py-20 text-center bg-white/70 dark:bg-zinc-900/75 backdrop-blur-2xl rounded-3xl border border-slate-200/70 dark:border-white/8 shadow-sm">
      <a-spin description="正在加载 API 资源与配额数据..." />
    </div>

    <!-- 暂无数据空状态 -->
    <div v-else-if="filteredKeys.length === 0" class="py-20 bg-white/70 dark:bg-zinc-900/75 backdrop-blur-2xl rounded-3xl border border-slate-200/70 dark:border-white/8 p-8 text-center shadow-sm">
      <a-empty :description="searchQuery || filterType !== 'all' || filterProvider !== 'all' ? '未找到符合条件的 API 资源记录' : '暂无配置记录'">
        <a-button type="primary" @click="openAddModal" class="mt-3! rounded-xl! active:scale-[0.98]">
          新建 API 资源
        </a-button>
      </a-empty>
    </div>

    <!-- 扁平卡片 Grid (3 列响应式网格，升级 Apple Depth 边框与 Ambient Glow 阴影) -->
    <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
      <div
        v-for="item in filteredKeys"
        :key="item.id"
        class="rounded-3xl border border-slate-200/70 dark:border-white/8 bg-white/95 dark:bg-zinc-900/90 shadow-[0_8px_30px_rgba(0,0,0,0.03)] dark:shadow-[0_8px_30px_rgba(0,0,0,0.35)] hover:shadow-[0_16px_40px_rgba(5,150,105,0.12)] hover:border-emerald-300/80 dark:hover:border-emerald-600/50 hover:-translate-y-1 transition-all duration-300 ease-out backdrop-blur-md overflow-hidden flex flex-col justify-between p-4 space-y-3"
      >
                ...
                <div
                  :class="[
                    ...
                    item.provider === 'google-antigravity'
                      ? 'bg-linear-to-br from-indigo-500 via-indigo-600 to-purple-600 shadow-indigo-500/25'
                      : 'bg-linear-to-br from-emerald-500 via-teal-600 to-cyan-600 shadow-emerald-500/25'
                  ]"
                >
                ...
                <span
                  :class="[
                    ...
                    item.provider === 'google-antigravity'
                      ...
                      : 'bg-emerald-50 dark:bg-emerald-950/70 text-emerald-600 dark:text-emerald-400 border-emerald-200/60 dark:border-emerald-800/60'
                  ]"
                >
                  <Zap
                    :class="[
                      'w-3.5 h-3.5',
                      item.provider === 'google-antigravity' ? 'text-indigo-500' : 'text-emerald-500'
                    ]"
                  />
                  Token Plane
                </span>
        ...
        <template v-else>
          <div class="space-y-4">
            ...
            <div v-if="item.quotaInfo" class="pt-2 grid grid-cols-3 gap-2 text-center">
              <div class="p-2 rounded-xl bg-cyan-50/50 dark:bg-cyan-950/30 border border-cyan-100/60 dark:border-cyan-900/60">
                <span class="text-[10px] font-semibold text-cyan-500 uppercase tracking-wider">剩余 RPM</span>
                <div class="text-sm font-extrabold font-mono text-cyan-600 dark:text-cyan-400 mt-0.5">
